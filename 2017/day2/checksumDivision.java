...
/**
 * Write a description of class checksumDivision here.
 *
 * @author (your name)
...
{
    public static int getQuotient(int[] intList)
    {   
            int quotient = 0;
            int dividend = intList[0];
            int divisor = intList[0];
            //2. 
            for(int i = 0; i < intList.length; i++)
            {
                for(int a = i+1; a < intList.length; a++)
                {
                    if(intList[i] % intList[a] == 0)
                    {
                        divisor = intList[a];
                        dividend = intList[i];
                        
                    }else if(intList[a] % intList[i] == 0)
                    {
                        divisor = intList[i];
                        dividend = intList[a];
    ...
    {
        //test, 
        //String[] inputList = {"5 1 9 5", "7 5 3", "2 4 6 8"};
        String[] inputList = {"1919 2959    82  507 3219    239 3494    1440    3107    259 3544    683 207 562 276 2963",
        ...
        //1. turn into int arrays
        //1.1 trim into smaller substrings
        for(int i=0; i < inputList.length; i++)
        {
            String[] formattedList = inputList[i].split("\\s+");
            int[] intList = new int[formattedList.length];
            //transform string to int, fill int array
            for (int a = 0; a < formattedList.length; a++) {
                //parse and store each value into int[] to be returned
                intList[a] = Integer.parseInt(formattedList[a]); 
            }
            sumOfDivisions += getQuotient(intList);
        }
        
        
        System.out.println("Result: " + sumOfDivisions);
